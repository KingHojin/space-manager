...
function Progress({ task, currentMinute, label }) {
  const raw = task.progress !== undefined
    ? task.progress * 100
    : ((currentMinute - (task.startedAt ?? currentMinute)) / Math.max(1, task.duration ?? 1)) * 100;
  ...
  return (
    <div className="mt-3 rounded-xl border border-cyan-400/30 bg-cyan-400/10 p-3">
      ...
      <div className="mt-2 flex flex-wrap gap-1.5 text-xs">
        <span className="hud-chip">{completeAt ? `완료 ${formatGameDate(completeAt)}` : "슬롯 대기"}</span>
...
function InjuryProgress({ injury }) {
  ...
  return (
    <div className="mt-3 rounded-xl border border-red-400/25 bg-red-400/10 p-3">
      <div className="mb-1 flex items-center justify-between text-xs">
        <span className="hud-label">{label} 회복률</span>
        <span className="hud-value">{Math.round(normalized.recoveryProgress ?? 0)}%</span>
      </div>
      <div className="hud-gauge hud-gauge-warn">
        <span className="hud-gauge-fill" style={{ width: `${normalized.recoveryProgress ?? 0}%` }} />
      </div>
      <div className="mt-2 flex flex-wrap gap-1.5 text-xs">
  ...
  return (
    <div className="mt-3 grid grid-cols-5 gap-1.5 text-xs">
      {entries.map(([key, label, value]) => (
        <div key={key} className={`rounded-xl border border-slate-700/70 bg-slate-950/55 p-2 text-center ${needTone(key, value)}`}>
          <div className="font-black tabular-nums">{Math.round(value)}</div>
          <div className="mt-0.5 truncate text-[10px] text-slate-400">{label}</div>
        </div>
      ))}
    </div>
  );
    ...
    <div className="mt-3 rounded-xl border border-sky-400/30 bg-sky-400/10 p-3 text-sm">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="hud-label">AI ORDER</div>
          <div className="mt-1 truncate font-semibold text-slate-100">{activity.station} · {activity.action}</div>
        </div>
...
export default function Crew() {
  ...
  const recoveryQueue = useMemo(
    () => rawJobs.filter((job) => activeJobStatuses.has(job.status)).map(jobToLegacyRecovery).filter(Boolean),
    [rawJobs],
  );
  ...
  const train = (member) => {
    ...
    return addLog(`${member.name} 훈련 시작: ${statLabel[statKey] ?? statKey} +1, 우선순위 ${getPriorityConfig(priority).label}, ₢${TRAINING_COST}, 완료 ${formatGameDate(completeAt)}.`);
  };
  ...
  const treat = (member) => {
    ...
    return addLog(`${member.name} 치료 시작: ${injuryLabel(member.injury)}, 우선순위 ${getPriorityConfig(priority).label}, ₢${rule.cost}, ${formatMinutes(rule.minutes)}, 완료 ${formatGameDate(completeAt)}.`);
  };
    ...
    <div className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
      <section>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">승무원 상태를 카드/컨디션 중심으로 확인합니다.</p>
          </div>
          <div className="flex flex-wrap justify-end gap-1.5">
            <span className="hud-chip hud-chip-accent">AI {aiSummary.total}</span>
            <span className="hud-chip">긴급 {aiSummary.emergency}</span>
            <span className="hud-chip">부상 {injuredCrewCount}</span>
          </div>
        </div>

        {missingRoles.length > 0 && (
          <div className="mt-3 rounded-xl border border-amber-300/35 bg-amber-300/10 p-3 text-sm leading-6 text-amber-100">
            역할 공백: {missingRoles.join(", ")}
          </div>
        )}
                ...
                <div className="mt-3 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="truncate font-black text-slate-100">{member.name}</div>
                    </div>
                    <div className="mt-1 truncate text-xs text-slate-500">{member.role} · {member.trait ?? "일반 대원"}</div>
                  </div>
      ...
      <div className="grid gap-4">
        ...
        <section className="rounded-2xl border border-slate-700/70 bg-slate-950/60 p-4">
          <div className="section-title">함내 상황</div>
          <div className="mt-3 grid grid-cols-3 gap-2 text-sm">
            <Info label="방" value={roomCount} />
            <Info label="위기" value={activeCrises.length} />
            <Info label="AI 로그" value={crewActivityLog?.length ?? 0} />
          </div>
        </section>
