...
function InjuryProgress({ injury }) {
  ...
  return (
    <div className="mt-3 rounded border border-red-400/25 bg-red-400/10 p-3">
    ...
    <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
      <section>
        ...
        <div className="mt-4 grid grid-cols-4 gap-2 text-sm">
          ...
          <Info label="역할 공백" value={`${coverage.missingRoles.length}개`} tone={coverage.missingRoles.length > 0 ? "text-amber-300" : ""} />
          <Info label="부상" value={`${crew.filter((member) => member.alive && isInjured(member.injury)).length}명`} />
        </div>
        {coverage.missingRoles.length > 0 && (
          <div className="mt-3 rounded border border-amber-300/35 bg-amber-300/10 p-3 text-sm leading-6 text-amber-100">
            역할 공백: {coverage.missingRoles.join(", ")} · 해당 역할 승무원이 중상 이상이면 함선 페널티가 발생합니다.
          </div>
        )}
        <div className="mt-4 rounded border border-slate-700/70 bg-slate-950/60 p-4 text-sm leading-6 text-slate-300">
          승무원은 함선 내부에서 AI 명령에 따라 이동합니다. 경상은 작업 가능하지만 효율이 떨어지고, 중상 이상은 모든 슬롯에서 제외됩니다.
        </div>
            ...
            return (
              <div key={member.id} className={`rounded border p-4 ${member.alive ? "border-slate-700/70 bg-slate-950/60" : "border-red-900/70 bg-red-950/20 opacity-80"}`}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`hud-chip ${injuryTone(member.injury, member.alive)}`}>{!member.alive ? "전사" : injuryLabel(member.injury)}</span>
                </div>
                ...
                <div className="mt-4 grid grid-cols-3 gap-2">
                  <button className="secondary-button" disabled={!member.alive || isBusy || !isHealthy(member.injury) || resources.credits < TRAINING_COST} onClick={() => train(member)}>{trainingTask ? "훈련 중" : treatmentTask ? "치료 중" : `훈련 ₢${TRAINING_COST}`}</button>
                  <button className="secondary-button" disabled={!member.alive || isBusy} onClick={() => rest(member)}>휴식</button>
                  <button className="secondary-button" disabled={!member.alive || !isInjured(member.injury) || isBusy || resources.credits < rule.cost} onClick={() => treat(member)}>{treatmentTask ? "치료 중" : !isInjured(member.injury) ? "정상" : `치료 ₢${rule.cost}`}</button>
                </div>
              </div>
            );
          })}
