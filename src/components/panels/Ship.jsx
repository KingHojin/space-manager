...
export default function Ship() {
  ...
  const equip = (slot, module) => {
    if (!unlocked.includes(module.id)) {
      addLog(`${module.name} 모듈은 아직 보유하지 않았습니다. 시장에서 구매하거나 제작하세요.`);
      return;
    }
    if (slotTask(slot)) {
      addLog(`${slot} 슬롯은 이미 장착 작업 중입니다.`);
      return;
    }
    const rule = getModuleRule(module);
    if (!spendCredits(rule.installCredits)) {
      addLog(`${module.name} 장착 실패: 크레딧이 부족합니다.`);
      return;
    }
  ...
  const upgrade = (module) => {
    ...
    if (moduleTask(module.id)) {
      addLog(`${module.name} 개선 실패: 이미 작업 중입니다.`);
      return;
    }
    const rule = getModuleRule(module);
    const materialQty = upgradeMaterialQty[module.rarity] ?? 2;
    if (getItemQty(items, "tritanium") < materialQty) {
      addLog(`${module.name} 개선 실패: 트리타늄 ${materialQty}개가 필요합니다.`);
      return;
    }
    if (!spendCredits(rule.upgradeCredits)) {
      addLog(`${module.name} 개선 실패: 크레딧이 부족합니다.`);
      return;
    }
    ...
    <div className="grid gap-4 xl:h-full xl:grid-cols-[0.9fr_1.1fr]">
      <section>
        ...
        <div className="mt-4 rounded border border-slate-700/70 bg-slate-950/60 p-4 text-sm leading-6 text-slate-300">
          모듈 장착과 개선은 즉시 완료되지 않습니다. 크레딧과 작업 시간이 필요하며, 게임 시간이 진행되면 자동 완료됩니다.
        </div>
          ...
          {MODULE_SLOTS.map((slot) => {
            ...
            return (
              <div key={slot} className={`ship-slot ship-slot-${slot.replace("-", "")}`}>
                ...
                <strong>{task ? "장착 작업 중" : module?.name}</strong>
                <small className="text-slate-400">{task ? formatGameDate(task.completeAt) : `Lv.${module?.level ?? 1}`}</small>
              </div>
            );
          })}
        </div>
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          <div className="min-w-0">
                            ...
                            <div className="mt-2 flex flex-wrap gap-1.5 text-xs">
                              <span className="hud-chip">장착 ₢{rule.installCredits}</span>
                              <span className="hud-chip">{formatMinutes(rule.installMinutes)}</span>
                              <span className="hud-chip">개선 ₢{rule.upgradeCredits}</span>
                              <span className="hud-chip">트리타늄 {materialQty}</span>
                            </div>
                          </div>
                          <div className="flex flex-col items-end gap-1">
                            <Badge rarity={module.rarity}>{module.rarity}</Badge>
                            <span className={`hud-chip ${owned ? "hud-chip-success" : ""}`}>{owned ? "보유" : "미보유"}</span>
                            {task && <span className="hud-chip hud-chip-warn">작업 중</span>}
                          </div>
                        </div>
                        {task && <Progress task={task} currentMinute={currentMinute} />}
                        <div className="mt-3 grid grid-cols-2 gap-2">
                          <button className="secondary-button" disabled={equipped || !owned || Boolean(currentSlotTask) || resources.credits < rule.installCredits} onClick={() => equip(slot, module)}>
                            {equipped ? "장착 중" : owned ? "장착 예약" : "구매 필요"}
                          </button>
                          <button className="secondary-button" disabled={!canUpgrade} onClick={() => upgrade(module)}>
                            개선 예약
                          </button>
                        </div>
                      </div>
  ...
  return (
    <div className="mt-3 rounded border border-cyan-400/30 bg-cyan-400/10 p-3">
      <div className="mb-1 flex items-center justify-between text-xs">
        <span className="hud-label">작업 진행</span>
