...
import { settleIncidentChoice, getIncidentOptionAvailability, getIncidentPresentation } from "../../orchestration/incidentDirectorOrchestrator";
    ...
    <section className={`rounded-2xl border p-4 ${runtime.severity === "medium" ? "border-red-400/50 bg-red-400/10" : "border-amber-300/40 bg-amber-300/10"}`}>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        {(template.options ?? []).map((option) => {
          const availability = getIncidentOptionAvailability(runtime, option);
          ...
          const forecast = option.job ? estimateIncidentJobTiming({ option, runtime, currentMinute, jobs, rooms, crew }) : null;
          ...
          return (
            <div key={option.id} className="rounded-2xl border border-slate-700/70 bg-slate-950/65 p-3">
              ...
              <button className="primary-button mt-3 w-full justify-center" disabled={!availability.ok || settlementLock.current} onClick={() => choose(option)}>{settlingOptionId === option.id ? "처리 중" : availability.ok ? "결재" : "선택 불가"}</button>
            </div>
          );
        })}
      </div>
