...
// A "tier" is deliberately translated into concrete authored aftermath, not
// a cosmetic label.  One mitigation tier shaves two points from the harmful
// numeric part of a failed response (and one crisis severity where present).
function mitigateFailureEffects(effects = [], failureTier = 0) {
  const amount = Math.max(0, -Number(failureTier ?? 0)) * 2;
  if (!amount) return effects;
  const improve = (value, harmfulWhen) => harmfulWhen(value) ? value + (value < 0 ? amount : -amount) : value;
  return effects.map((effect) => {
    if (effect.type === "resources") return { ...effect, delta: Object.fromEntries(Object.entries(effect.delta ?? {}).map(([key, value]) => [key, improve(value, (entry) => entry < 0)])) };
    if (effect.type === "room") return { ...effect, condition: improve(effect.condition, (entry) => entry < 0), load: improve(effect.load, (entry) => entry > 0) };
    if (["crewAll", "targetCrew", "targetPair"].includes(effect.type)) return { ...effect, fatigue: improve(effect.fatigue, (entry) => entry > 0), morale: improve(effect.morale, (entry) => entry < 0), needs: Object.fromEntries(Object.entries(effect.needs ?? {}).map(([key, value]) => [key, improve(value, (entry) => entry > 0)])) };
    if (effect.type === "crisis") return { ...effect, severity: Math.max(0, Number(effect.severity ?? 1) - Math.max(1, -Number(failureTier))) };
    return effect;
  }).filter((effect) => effect.type !== "crisis" || effect.severity > 0);
}

function addCompletionRelief(effects = [], modifiers = {}, context = null) {
  // Rest brace is contextual to the watch/sleep-debt response.  Its fatigue
  // modifier becomes an explicit sleep-debt relief on the pinned target.
  if (context !== "crew" || !Number(modifiers.fatigueDelta ?? 0)) return effects;
  return [...effects, { type: "targetCrew", needs: { sleepDebt: Number(modifiers.fatigueDelta) } }];
}
...
export function getIncidentLeadCandidates(runtime, option) {
  if (!option?.job) return [];
  const template = getDirectorIncident(runtime?.templateId);
  const contextByCategory = { engineering: "engineering", navigation: "scouting", supplies: "cooking", crew: "crew", "life-support": "engineering", opportunity: "command" };
  const context = option.job.context ?? contextByCategory[template?.category] ?? "engineering";
  const threshold = option.job.threshold ?? (option.job.duration >= 180 ? 14 : 10);
  const instances = useEquipmentStore.getState().instances ?? [];
  return useCrewStore.getState().crew
    .filter((member) => isUsableCrew(member) && (!option.job.requiredRole || member.role === option.job.requiredRole))
    .map((member) => prepareCrewLead({ member, context, threshold, equipment: equipmentForCrew(instances, member.id) }));
}

function preflightSnapshot(runtime, option, leadCrewId = null, useSpecialty = false) {
  ...
  if (option.job) {
    ...
    // UI always asks for a lead.  The deterministic fallback only preserves
    // legacy integrations/tests that predate the agency selector.
    const candidates = getIncidentLeadCandidates(runtime, option);
    const legacyLead = !leadCrewId;
    const lead = candidates.find((entry) => entry.leadCrewId === leadCrewId) ?? (legacyLead ? candidates[0] : null);
    if (!lead?.profile?.usable) return { ok: false, reason: "leadUnavailable", candidates: getIncidentLeadCandidates(runtime, option) };
    ...
    const modifiers = legacyLead ? { durationMinutes: 0, fatigueDelta: 0, penaltyTier: 0, failureTier: 0, resourceDelta: {} } : projectActionModifiers(lead);
    const sectorId = useNavStore.getState().sector?.id ?? "sector";
    const specialty = useSpecialty ? specialtyAvailability({ member: workers.find((member) => member.id === lead.leadCrewId), sectorId, context: lead.context, profile: lead.profile }) : null;
    if (useSpecialty && !specialty?.ok) return { ok: false, reason: `specialty:${specialty?.reason ?? "unavailable"}` };
    const specialtyDuration = specialty?.specialty?.id === "signal-separation" ? -60 : 0;
    const duration = Math.max(30, (option.job.duration ?? 0) + modifiers.durationMinutes + specialtyDuration);
    const belowLoad = Math.max(0, 2 + Number(modifiers.penaltyTier ?? 0));
    const effects = lead.tier === "below" ? [...(option.effects ?? []), { type: "room", roomId: option.job.roomId, load: belowLoad }] : option.effects ?? [];
    const specialtyEffects = specialty?.specialty?.id === "bypass-wiring" ? [{ type: "room", roomId: "engineering", load: -6 }]
      : specialty?.specialty?.id === "rationing" ? [{ type: "crewAll", needs: { hunger: -3 } }]
        : [];
    const specialtyCompletionEffects = specialty?.specialty?.id === "shift-coordinator" ? [{ type: "targetCrew", needs: { sleepDebt: -8, stress: -2 } }] : [];
    return { ok: true, waitText, snapshot: { effects: [...effects, ...specialtyEffects], costs, job: { ...option.job, duration, completionEffects: [...addCompletionRelief(option.job.completionEffects ?? [], modifiers, lead.context), ...specialtyCompletionEffects], failureEffects: mitigateFailureEffects(option.job.failureEffects ?? [], modifiers.failureTier) }, label: option.label, detail: option.detail, targets: runtime.targets, lead: { ...lead, modifiers }, specialty: specialty ? { id: specialty.specialty.id, crewId: lead.leadCrewId, sectorId, deadlineBonus: ["bypass-wiring", "steady-command"].includes(specialty.specialty.id) ? 90 : 0 } : null } };
  }
  return { ok: true, waitText, snapshot: { effects: option.effects ?? [], costs, job: null, label: option.label, detail: option.detail, targets: runtime.targets, lead: null } };
}

export function getIncidentOptionAvailability(runtime, option, leadCrewId = null, useSpecialty = false) {
  return preflightSnapshot(runtime, option, leadCrewId, useSpecialty);
}
...
function settlePrepared(runtimeId, currentMinute, afterStep) {
  ...
  if (pending.snapshot.specialty) {
    const claim = useCrewStore.getState().claimSpecialtyUse({ crewId: pending.snapshot.specialty.crewId, sectorId: pending.snapshot.specialty.sectorId, claimId: `${pending.claimId}:specialty` });
    if (!claim.ok && !claim.repeated) return claim;
    if (pending.snapshot.specialty.deadlineBonus > 0 && !pending.snapshot.specialty.deadlineBonusApplied) useIncidentStore.setState((state) => {
      const current = state.runtimesById[runtimeId];
      if (!current?.pendingClaim?.snapshot?.specialty || current.pendingClaim.snapshot.specialty.deadlineBonusApplied) return state;
      return { runtimesById: { ...state.runtimesById, [runtimeId]: { ...current, deadlineAtMinute: (current.deadlineAtMinute ?? currentMinute) + current.pendingClaim.snapshot.specialty.deadlineBonus, pendingClaim: { ...current.pendingClaim, snapshot: { ...current.pendingClaim.snapshot, specialty: { ...current.pendingClaim.snapshot.specialty, deadlineBonusApplied: true } } } } } };
    });
  }
  ...
  if (pending.snapshot.job) {
    ...
    const leadCrewId = pending.snapshot.lead?.leadCrewId ?? null;
    const jobResult = useJobStore.getState().applyIncidentJob(pending.claimId, { id: jobId, type: "incident_response", roomId: spec.roomId, duration: spec.duration, assignedCrewId: leadCrewId, requiredRole, priority: runtime.severity === "medium" ? "high" : "normal", createdAt: currentMinute, payload: { targetCrewId: leadCrewId, incident: { runtimeId, claimId: pending.claimId, leadCrewId, completionEffects: spec.completionEffects ?? [], failureEffects: spec.failureEffects ?? [], label: pending.snapshot.label, lead: pending.snapshot.lead ?? null } } });
    afterStep?.("job");
...
export function settleIncidentChoice({ runtimeId, stageId, claimId, optionId, leadCrewId = null, useSpecialty = false, manual = false, currentMinute = useGameStore.getState().currentMinute, afterStep } = {}) {
  ...
  const preflight = preflightSnapshot(runtime, option, leadCrewId, useSpecialty);
