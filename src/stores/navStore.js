...
function withNodeFlags(sector, visitedIds, discoveredIds) {
  ...
  return {
    ...sector,
    nodes: sector.nodes.map((node) => ({ ...node, visited: visited.has(node.id), discovered: discovered.has(node.id) })),
  };
}
    ...
    (set, get) => {
      ...
      return {
        ...
        planRoute: (targetNodeId, currentMinute = 0) => {
          ...
          if (state.fuel <= 0) return { ok: false, reason: "drifting" };
        ...
        tickTravel: (deltaMinutes = 0, currentMinute = 0) => {
          ...
          if (state.fuel <= 0) {
            const driftState = state.driftState ?? { startedAt: currentMinute, rescueEta: currentMinute + 240 };
            set({ driftState, travel: null, navLog: ["연료 고갈: 함선이 표류 상태에 들어갔습니다.", ...state.navLog].slice(0, 10) });
            return { effects: [{ kind: "log", message: "연료 고갈로 표류 상태가 되었습니다." }], logs: ["연료 고갈로 표류 상태가 되었습니다."] };
          }
        ...
        refuel: (amount = 100) => set((state) => ({ fuel: clamp(state.fuel + amount, 0, 100), driftState: state.fuel + amount > 0 ? null : state.driftState })),
        ...
        getNavCard: () => {
          ...
          if (state.driftState) return { mode: "drift", priority: "critical", title: "표류 상태", desc: "연료가 없어 이동할 수 없습니다. 정거장 구조나 긴급 보급이 필요합니다.", meta: "연료 0" };
    ...
    },
    {
      ...
      merge: (persistedState, currentState) => {
        ...
        return {
          ...
          discovered,
          visited,
          route: persistedState?.route ?? [currentNodeId],
          travel: persistedState?.travel ?? null,
          fuel: clamp(persistedState?.fuel ?? 100, 0, 100),
