...
>
> 최종 갱신: 2026-07-09 (Phase 20 완료 + 버그 라운드 21 시점, 테스트 420개 통과)
...
- **아이콘**: lucide-react
- **테스트**: vitest (node 환경, `tests/setup.js`가 localStorage/window 스텁 제공) — **현재 420개 전부 통과**
...
### 🥇 Phase 21 — Inner Life (다음 권장 작업)
`PHASE_13_LIVING_CREW.md`의 Layer B 실행. 승무원을 "방 사이를 오가는 마커"에서 "성격 있는 존재"로.
- **21-A**: 성격 특성(traits) 카탈로그(data) + crew 필드 확장(세이브 호환 merge) — 표시만, 효과 없음
- **21-B**: 무드 모델(needs/사건 → 무드) + 무드가 작업 속도·방작업 점수에 **소폭(±10~15%)** 영향(순수 함수, 기존 공식에 곱연산)
- **21-C**: 관계(친밀/불화) — 같은 방 근무·식사로 변화, 불화 시 효율 페널티
...
4. `npm test`를 돌려 420개 통과를 확인한다. 테스트 파일들(`src/**/__tests__/`)이 각 시스템의 계약을 보여준다.
