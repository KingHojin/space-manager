...
>
> 최종 갱신: 2026-07-09 (Phase 21-C 완료: 관계 상태 + 공유 작업 효율, 테스트 435개 통과)
...
- **아이콘**: lucide-react
- **테스트**: vitest (node 환경, `tests/setup.js`가 localStorage/window 스텁 제공) — **현재 435개 전부 통과**
...
> ⚠️ **2026-07-10 방향 전면 교정** — 상세 근거는 `docs/GAME_DIRECTION_AUDIT.md` (필독).
> 감사 결과: 시스템은 많은데 **패배 조건·승리 목표·난이도 진행이 전부 부재**하고
> 스킬트리 등 고립 시스템이 쌓임. 이에 따라 **신규 시뮬레이션 시스템 동결**,
> "게임을 게임으로 만드는" Phase 22를 최우선으로 재편.

### 🥇 Phase 22 — Make It A Game (최우선, 신규 시스템 동결 하에 진행)
상세 설계: `docs/GAME_DIRECTION_AUDIT.md` 3부.
- **22-A 패배**: hull 0 = 함선 파괴 게임오버 / oxygen 0 = 질식 진행 / 전원 사망 = 게임오버 / 표류 무료 탈출 제거
- **22-B 목표·진행**: 캠페인 목표(예: 섹터 5 최종 관문 = 1차 엔딩) + `generateSector(seed, sectorIndex)` 난이도 스케일링 + 홈 "현재 목표" 카드
- **22-C 고립 시스템 연결**: 스킬트리 → 실효과 연결(1순위), 사냥 경제 편입 재검토(PR #76 방향 포함)
- **22-D 온보딩**: 첫 10분 가이드 + 게임오버/승리 화면
- **22-E 재미 검증 루프**: 게임플레이 PR 완성 정의에 "플레이 검증" 추가 + 오너 플레이테스트 마일스톤

### 🥈 Phase 21 잔여 — Inner Life 후속 (22 완료 후 재개)
21-A~C 골격은 머지됨(PR #98). 후속(가시화·드라마·특성 기계화)은 `PHASE_21_REVIEW_REPORT.md`의
수정 계획(21-D′~G′)을 따르되 **Phase 22 이후로 보류** — 죽지 않는 게임에 감정 시스템부터 붙이는 것은 순서 오류.

### 🥉 그 다음
- 전투↔함내 연결(전투 피해 → hull_breach/intruder 위기 스폰, `PHASE_11` 미완 항목 — 22-A와 자연 연계 가능)
- 소형 백로그: economy 보고서 생성기 / 위기 escalation 쓰로틀 보고 / fuel-reserve 자동 구매 / 전역 focus-visible

### 🏆 대형 (마지막)
함대(Fleet) 활성화 — `shipStore.vesselsById` 실전화. 전투/임무는 이미 vesselId 스코프.
...
4. `npm test`를 돌려 435개 통과를 확인한다. 테스트 파일들(`src/**/__tests__/`)이 각 시스템의 계약을 보여준다.
