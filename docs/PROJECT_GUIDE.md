...
>
> 최종 갱신: 2026-07-09 (Phase 21-A 진행: Inner Life 성격 특성 표시 기반, 테스트 424개 통과)
...
- **아이콘**: lucide-react
- **테스트**: vitest (node 환경, `tests/setup.js`가 localStorage/window 스텁 제공) — **현재 424개 전부 통과**
...
- **21-B 다음**: 무드 모델(needs/사건 → 무드) + 무드가 작업 속도·방작업 점수에 **소폭(±10~15%)** 영향(순수 함수, 기존 공식에 곱연산)
...
4. `npm test`를 돌려 424개 통과를 확인한다. 테스트 파일들(`src/**/__tests__/`)이 각 시스템의 계약을 보여준다.
