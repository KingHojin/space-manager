...
>
> 최종 갱신: 2026-07-09 (Phase 21-C 진행: 관계 상태 기반, 테스트 433개 통과)
...
- **아이콘**: lucide-react
- **테스트**: vitest (node 환경, `tests/setup.js`가 localStorage/window 스텁 제공) — **현재 433개 전부 통과**
...
- **21-C 진행**: 관계 상태(`crewStore.relationships`) + 같은 방 근무·식사 기반 친밀도 변화. 다음 후속은 불화 효율 페널티
...
4. `npm test`를 돌려 433개 통과를 확인한다. 테스트 파일들(`src/**/__tests__/`)이 각 시스템의 계약을 보여준다.
